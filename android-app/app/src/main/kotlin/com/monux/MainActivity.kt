...
private fun MainScreen() {
    ...
    val toggles = listOf(
        ...
        FeatureToggle("screen", "投屏", state.featureFlags.screen, if (state.screen.enabled) "scrcpy 正在运行" else "Quick Settings Tile / UI 控制"),
    )
    ...
    LazyColumn(
        ...
        item {
            ScreenControlCard(state.screen.maxSize, state.screen.bitrate, state.screen.enabled)
        }
        items(toggles) { item ->
...
private fun StatusCard(linuxName: String, linuxIp: String, status: String) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(20.dp)) {
            Text("Monux", style = MaterialTheme.typography.headlineSmall)
...
@Composable
private fun ScreenControlCard(initialMaxSize: Int, initialBitrate: String, enabled: Boolean) {
    var maxSize by remember(initialMaxSize) { mutableStateOf(initialMaxSize.toString()) }
    var bitrate by remember(initialBitrate) { mutableStateOf(initialBitrate) }

    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            Text("投屏设置", style = MaterialTheme.typography.titleMedium)
            OutlinedTextField(
                value = maxSize,
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                Button(onClick = {
                    MainService.updateScreenConfig(maxSize.toIntOrNull() ?: 1600, bitrate)
                    if (!enabled) MainService.toggleScreenMirror()
                }) {
                    Text(if (enabled) "应用配置" else "开始投屏")
                }
                Button(onClick = {
                    if (enabled) MainService.toggleScreenMirror()
                }) {
